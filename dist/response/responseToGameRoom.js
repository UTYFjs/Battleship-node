...
export const responseToGameRoom = (type, currentIdGame, dataAttack) => {
    let dataResponse;
...
const checkStatusAttack = (dataAttack) => {
    ...
    if (currentGame) {
        ...
        if (currentGame[indexEnemies].field[y][x]) {
            ...
            if (currentGame[indexEnemies].shipsXY[indexShip].XY.length === 0 &&
                ...
                if (restShipCells === 0) {
                    if (currentGame && typeof currentGame.currentPlayer === 'number') {
                        const winner = winnersDb.find((winner) => winner.name === currentGame.currentRoom.roomUsers[currentGame.currentPlayer].name);
                        if (winner) {
                            winner.wins += 1;
                        }
                        else {
                            winnersDb.push({
                                name: currentGame.currentRoom.roomUsers[currentGame.currentPlayer].name,
                                wins: 1,
                            });
                        }
                    }
